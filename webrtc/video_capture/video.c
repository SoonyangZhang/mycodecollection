...
{  
	...
	now_ms=time_ms-last_time;
	}
    last_time=time_ms;
    char buffer[512]={0};
    memset(buffer,0,512);
    n=sprintf (buffer,"%ld %d %d",now_ms,num,length);
    if (fwrite(buffer, strlen(buffer), 1, video_log) != 1) {
        printf("write error");  
        exit(EXIT_FAILURE);  
    }
	char ch[2]="\n\r";
	fwrite(ch,2,1,video_log);
	num++;
    return 0;  
}  
...
{  
    ...
    if((video_log = fopen("video_log.txt", "w")) == NULL)  
    {  
        perror("Fail to fopen");  
        exit(EXIT_FAILURE);  
    } 
    /*char buffer[512]={0};
    sprintf(buffer,"%d %d hello fuck",12,14);
    fwrite(buffer,strlen(buffer),1,video_log);
	char ch[2]="\n\r";
	fwrite(ch,2,1,video_log);
    memset(buffer,0,512); 
    sprintf(buffer,"%d %d hello fuck",12,14);
